...
# دالة مساعدة لتحويل ملفات PDF إلى صور PNG تلقائياً
def convert_to_image(uploaded_file, output_folder, base_name):
    file_ext = uploaded_file.name.split('.')[-1].lower()
    file_path = os.path.join(output_folder, f"{base_name}.{file_ext}")
    ...
    if file_ext == "pdf":
        ...
        page = doc_pdf.load_page(0) # أول صفحة
        pix = page.get_pixmap(dpi=150)
        pix.save(img_output_path)
    else:
        # إذا كانت صورة أصلية، نقوم بنسخها بصيغة PNG موحدة
        import shutil
        shutil.copy(file_path, img_output_path)
...
if st.button("حفظ البيانات ومعالجة المرفقات 📁"):
    if not company_name:
        ...
        if cr_file:
            convert_to_image(cr_file, project_folder, "Commercial_Register")
        if vat_file:
            convert_to_image(vat_file, project_folder, "VAT_Certificate")
        if address_file:
            convert_to_image(address_file, project_folder, "National_Address")
                
        st.success("✅ تم حفظ ومعالجة جميع المستندات الرسمية بنجاح!")
...
if st.button("إنشاء مسودة التقرير (Word) 📝"):
    if not company_name:
        ...
        with st.spinner('⏳ جاري أتمتة الخريطة وتجهيز المستندات في مواقعها...'):
            ...
            # تجهيز السياق والربط بالصور مباشرة بدون طباعة أسمائها
            context = {
            ...
            cr_img_path = os.path.join(project_folder, "Commercial_Register.png")
            vat_img_path = os.path.join(project_folder, "VAT_Certificate.png")
            addr_img_path = os.path.join(project_folder, "National_Address.png")
            
            if os.path.exists(cr_img_path):
                context['cr_image'] = InlineImage(doc_template, cr_img_path, width=Inches(5.0))
            else:
                context['cr_image'] = "[لم يتم إرفاق السجل التجاري]"

            if os.path.exists(vat_img_path):
                context['vat_image'] = InlineImage(doc_template, vat_img_path, width=Inches(5.0))
            else:
                context['vat_image'] = "[لم يتم إرفاق شهادة الضريبة]"

            if os.path.exists(addr_img_path):
                context['address_image'] = InlineImage(doc_template, addr_img_path, width=Inches(5.0))
            else:
                context['address_image'] = "[لم يتم إرفاق العنوان الوطني]"
        ...
        st.success("🎉 تم تعبئة التقرير وإدراج الصور في مواقعها المخصصة بنجاح تام!")
        st.download_button(
