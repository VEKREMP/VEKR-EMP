...
# ------------------------------------------------
# إعدادات واجهة الصفحة (العنوان والأيقونة والخلفية)
# ------------------------------------------------
st.set_page_config(
...
# ------------------------------------------------
# قسم المرفقات والمستندات الرسمية
# ------------------------------------------------
st.markdown("---")
...
facility_images = st.file_uploader("📸 صور المنشأة الإضافية", type=["png", "jpg", "jpeg"], accept_multiple_files=True)
contract_file = st.file_uploader("📄 عقد المخلفات الأساسي (PDF أو صورة)", type=["pdf", "png", "jpg", "jpeg"])

if st.button("حفظ البيانات ومعالجة المرفقات 📁"):
    if not company_name:
        st.error("الرجاء إدخال **اسم الشركة** أولاً لإنشاء المجلد.")
    else:
        project_folder = os.path.join("Projects", company_name)
        os.makedirs(project_folder, exist_ok=True)

        if cr_file:
            with open(os.path.join(project_folder, "Commercial_Register." + cr_file.name.split('.')[-1]), "wb") as f:
                f.write(cr_file.getbuffer())
        
        if vat_file:
            with open(os.path.join(project_folder, "VAT_Certificate." + vat_file.name.split('.')[-1]), "wb") as f:
                f.write(vat_file.getbuffer())

        if address_file:
            with open(os.path.join(project_folder, "National_Address." + address_file.name.split('.')[-1]), "wb") as f:
                f.write(address_file.getbuffer())

        if facility_images:
            for img in facility_images:
                with open(os.path.join(project_folder, img.name), "wb") as f:
                    f.write(img.getbuffer())
        
        if contract_file:
            contract_path = os.path.join(project_folder, contract_file.name)
            with open(contract_path, "wb") as f:
                f.write(contract_file.getbuffer())
            
            if contract_file.name.endswith(".pdf"):
                doc_pdf = fitz.open(contract_path)
                for page_num in range(len(doc_pdf)):
                    page = doc_pdf.load_page(page_num)
                    pix = page.get_pixmap(dpi=150)
                    pix.save(os.path.join(project_folder, f"Contract_Page_{page_num + 1}.png"))
                
        st.success("✅ تم حفظ جميع المستندات والمرفقات في مجلد المشروع بنجاح!")

# ------------------------------------------------
# قسم إصدار التقرير النهائي (عبر القالب)
# ------------------------------------------------
st.markdown("---")
...
if st.button("إنشاء مسودة التقرير (Word) 📝"):
    if not company_name:
        ...
        with st.spinner('⏳ جاري أتمتة الخريطة وإنشاء التقرير... الرجاء الانتظار قليلاً'):
            ...
            context = {
                ...
                'commercial_register': "[تم إرفاق السجل التجاري]",
                'vat_certificate': "[تم إرفاق شهادة ضريبة القيمة المضافة]",
                'national_address_doc': "[تم إرفاق مستند العنوان الوطني]"
            }
            
            if os.path.exists(map_path):
                context['map_image'] = InlineImage(doc_template, map_path, width=Inches(5.5))
            else:
                context['map_image'] = "[لم يتم تصوير خريطة الموقع]"
            
            doc_template.render(context)
            temp_io = io.BytesIO()
            doc_template.save(temp_io)
            temp_io.seek(0)
            
            final_doc = Document(temp_io)
            if os.path.exists(project_folder):
                final_doc.add_page_break()
                final_doc.add_heading('المرفقات والمستندات الرسمية', level=1)
                
                for filename in os.listdir(project_folder):
                    if filename.lower().endswith(('.png', '.jpg', '.jpeg')) and filename != "Map_Screenshot.png":
                        img_path = os.path.join(project_folder, filename)
                        final_doc.add_paragraph(f"مستند / مرفق: {filename}")
                        try:
                            final_doc.add_picture(img_path, width=Inches(5.0))
                        except:
                            pass
        ...
        st.success("🎉 تم التقاط الخريطة وتعبئة التقرير بنجاح تام!")
        st.download_button(
